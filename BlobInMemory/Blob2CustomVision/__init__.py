...
def csv_to_df(blob_service_client, csv_filename, container):
    '''
    ...
    Parameters
    ----------
    blob_service_client (BlobServiceClient): client to interact with blob storage at account level
    ...
    Returns
    -------
    df (DataFrame) : DataFrame mapping image filenames to grain-size labels
    '''
...
def get_tags(trainer, project):
    ...
    Parameters
    ----------
    trainer (CustomVisionTrainingClient) : client for interacting with Custom Vision
    project (Project) : relevant Custom Vision project
...
def tag_images(metadata_df, blob_service_client, container_name, trainer, project, base_image_url="ExampleDataset1/", multi_label='off'):
    ...
    Parameters
    ----------
    metadata_df (DataFrame) : DataFrame mapping image filenames to grain-size labels
    blob_service_client (BlobServiceClient): client to interact with blob storage at account level
    ...
    for index, row in metadata_df.iterrows():
        ...
        if '/' not in row[target_variable]:
            labels = [row[target_variable]]
        
        # Multi-label case
        elif '/' in row[target_variable] and multi_label == 'on':

            # Process values for target variable
            labels_temp = row[target_variable].split('/')
            labels = [s.strip() for s in labels_temp]
        ...
        for l in labels:
            ...
            if l not in tags:
                ...
                try:
                    new_tag = trainer.create_tag(project.id, l)
                    tags[l] = new_tag
                except:
                    pass
            
            tag_ids.append(tags[l].id)
        ...
        try:
            ...
            # Convert memory stream to bytearray (required by ImageFileCreateEntry)
            contents = bytearray(stream.getvalue())

            # Tag and add to image list
            image_list.append(ImageFileCreateEntry(name=filename, contents=contents, tag_ids=tag_ids))
...
def main(req: func.HttpRequest) -> func.HttpResponse:
    ...
    images_per_batch = 10
    list_of_dfs = [df.loc[i:i+images_per_batch-1,:] for i in range(0, len(df),images_per_batch)]
    ...
    for d in list_of_dfs:
        ...
        image_list = tag_images(d, blob_service_client, container, trainer, project)
